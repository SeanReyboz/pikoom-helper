...
client.on('ready', () => {
	...
	Cron.schedule('1 30 9 * * 1', async () => {
		...
		client.channels.fetch(botChannelId).then(channel => {
			channel.send(`:warning: **RAPPEL DEADLINE !!** :warning:\n\n**${deadline.title}**\n${deadline.description}\n\n> Date: __*${deadline.date}*__\n@everyone`);
		})
	});
...
client.on('interactionCreate', async interaction => {
	...
	if (commandName === 'ajouter') {
		...
		if (!regex.test(date)) {
			return interaction.reply(`La date doit être de la forme JJ-MM-AAAA. Vous avez saisi: ${date}.`);
		}
		...
		try {
			...
			return interaction.reply(`La deadline intitulée "${titre}" a bien été ajoutée!`);
		} catch (error) {
			if (error.name === 'SequelizeUniqueConstraintError') {
				return interaction.reply("L'identifiant est déjà utilisé.");
			}
			return interaction.reply("Oops! Une erreur s'est produite lors de l'ajout de la deadline.");	
		}
		...
		if (!rowCount) return interaction.reply(`La deadline "${id}" n'existe pas.`);
		...
		try {
			...
			if (len === 0) {
				return interaction.reply("Il n'y a aucune deadline (...pour l'instant ;) )");
			}
			...
			console.log(error);
			return interaction.reply("Oops! Quelque chose s'est mal passé lors de la récupération de la liste des deadlines.");
		}
...
**/aide** - Afficher ce message d'aide.
		...
		// convertir la date:
		deadline.date = moment(deadline.date).format('DD/MM/YYYY');
		interaction.reply(':warning: **RAPPEL DEADLINE !!** :warning:\n\n');
